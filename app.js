...
function reponsiveNav() {
  ...
  if (currentPosition < sections[1].offsetTop) {
    ...
    let progress =
      Math.ceil((currentPosition / sections[1].offsetTop) * 10) / 10;
    let opacity =
      Math.floor((currentPosition / sections[1].offsetTop) * 10) / 10;
    let rbg = [84, 72, 85, 1];
...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  typewriter
    .pauseFor(1500)
    .typeString(
      'I have a passion for designing and building <strong><span class="highlight">web experiences</span></strong>'
    )
    .pauseFor(1500)
    .deleteChars(n)
    .pauseFor(1000)
    .typeString(
      "am an <strong class='highlight'>Informatics</strong> major at the University of California, Irvine"
    )
    .pauseFor(1500)
    .deleteAll()
    .pauseFor(1000)
    .typeString(
      "Scroll down to learn more about me <strong class='highlight'>:)</strong>"
    )
    .pauseFor(1500)
    .start();
